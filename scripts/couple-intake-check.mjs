// End-to-end check of the wedding intake (npm run check:intake).
//
...
//
// Boots this repo's vite dev server, creates a couple over the DJ API, walks
// the couple wizard and the friends link in a real browser, verifies scope
// rules and autosave persistence, and screenshots every screen into
// .cache/couple-intake. Only the Spotify /search endpoint is mocked in the
// browser (no credentials needed); every save hits the real server. The couple
// is deleted at the end, so the DJ's data stays untouched.
//
// The DJ-side half of this check (the couples panel) lives in
// Vikteur/rekord-dj as scripts/couples-panel-check.mjs.
